...
describe('App component', () => {
  ...
  const getElementForTest1 = (ulId, liId) =>
    component.find('ul').at(ulId).find('li').at(liId);
  ...
  it('2. data map check', () => {
    ...
    expect(getElementForTest1(0, 0).text()).toBe(data[0].courseName);
    expect(getElementForTest1(0, 1).text()).toBe(data[0].courseDept);
    expect(getElementForTest1(0, 2).text()).toBe(data[0].description);
    expect(getElementForTest1(0, 4).text()).toBe(getCourseDetails(data[0]));

    expect(getElementForTest1(1, 0).text()).toBe(data[1].courseName);
    expect(getElementForTest1(1, 1).text()).toBe(data[1].courseDept);
    expect(getElementForTest1(1, 2).text()).toBe(data[1].description);
    expect(getElementForTest1(1, 4).text()).toBe(getCourseDetails(data[1]));

    expect(getElementForTest1(2, 0).text()).toBe(data[2].courseName);
    expect(getElementForTest1(2, 1).text()).toBe(data[2].courseDept);
    expect(getElementForTest1(2, 2).text()).toBe(data[2].description);
    expect(getElementForTest1(2, 4).text()).toBe(getCourseDetails(data[2]));
  });

  it('3. Add rating check', () => {
    component.instance().setState({ data });

    const addRating = getElementForTest1(0, 3)
      .find('li')
      .at(0)
      .find('.rate')
      .find('button');
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      `${backendApp}/courses/rating/${data[0]._id}`,
      {
        method: 'PATCH',
  ...
  it('5. drop rating check', () => {
    ...
    // addrating();
    const dropCourse = getElementForTest1(0, 3).find('.drop').find('button');
    dropCourse.simulate('click');
    // delete
